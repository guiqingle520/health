...
- 引入国际化基础能力，首期支持中文简体、中文繁体、英文、日文、韩文，并预留新增语言扩展流程。
...
- Next：App、Web 和后端统一使用 canonical locale：`zh-Hans`、`zh-Hant`、`en`、`ja`、`ko`。
- Next：客户端 UI 文案、错误提示、日期、数字、单位显示走本地化资源；后端返回稳定错误码。
- Next：AI 建议、报告、通知等生成类内容需要记录生成 locale，相关 Redis 缓存 key 必须包含 locale。
- Future：新增语言只增加翻译资源、locale allowlist、内容模板和 QA 截图，不修改业务 DTO。
